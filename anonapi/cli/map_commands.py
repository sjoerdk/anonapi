...
def init(context: MapCommandContext):
    ...
    options = \
        [RootSourcePath(context.current_path)] + get_initial_options(context.settings)
    mapping = Mapping(
...
def add_path_to_mapping_click(path: Path, mapping: Mapping,
    ...
    row = [SourceIdentifierParameter(file_selection_identifier),
           PatientName(patient_name), PatientID(patient_id), Description(description)]
...
def add_all_study_folders(context: MapCommandContext, pattern):
    ...
    """
    # Examples: **/* */1 */*/*2
    found = [x.relative_to(context.current_path)
             for x in Path(context.current_path).glob(pattern) if x.is_dir()]
    click.echo(f"Pattern '{pattern}' matches the following paths"
...
for func in [status, init, delete, add_study_folder, add_all_study_folders,
             edit, add_selection]:
