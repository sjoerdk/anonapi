...
def add(context: SelectCommandContext, pattern, recurse, check_dicom,
    ...
    selection = selection_folder.load_file_selection_or_new(
        description=selection_folder.path.name + " auto-generated by anonapi",
        selected_paths=paths,
    )

    # if there was a selection before, see whether new files have been found
    already_selected = selection.selected_paths_absolute
    paths_new = list(set(paths).difference(set(already_selected)))
    selection.selected_paths = selection.make_relative(
        selection.selected_paths + paths_new
    )
    selection_folder.save_file_selection(selection)

    click.echo(f"added {len(paths_new)} files")
